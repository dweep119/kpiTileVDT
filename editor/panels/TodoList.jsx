...
const panelConfig = {
 ...
 class TodoList extends Component{
    ...
    createTasks(item) {
      return <div className="list-style" ><span key={item.key} style={{textTransform: "capitalize"}}>{item.text}</span><div><i style={{margin: "0px 10px"}} className="material-icons" onClick={() => this.proceed(item.key, item.text)}>settings</i><i style={{margin: "0px 10px"}} className="material-icons" onClick={() => this.deleteItem(item.key)} >delete</i></div></div>
    }
    ...
    render() {
      ...
      var todoEntries = toJS(store.get('kpitile'));    
                ...
                <Flex flexDirection={"column"} className={"menu-data-view"}>
                  ...
                  {this.state.tabSelected == "general" &&
                    <div className="vdt-general">
                      <GeneralComponent  store={this.props.store}
                        editorData={TodoList}
                        expand={this.expand}
                        data={this.state.key}
                        onSubmit={(key)=> this.setState({ tabSelected: 'appearance', key: key})}
                        />
                    </div>
                  }
                  {this.state.tabSelected == "appearance" &&
                    <div className="vdt-appearance">
                      <AppearanceComponent store={this.props.store}
                        component={this.state.component}
                        editorData={TodoList}
                        expand={this.expand}
                        data={this.state.key}
                        onSubmit={(key)=> this.setState({ tabSelected: 'data', key: key})}/>
                    </div>
                  }
                  {this.state.tabSelected == "data" &&
                    <div className="vdt-data">
                      <DataComponent  store={this.props.store}
                        data={this.state.key}
...
export default withPanel(observer(TodoList), panelConfig);
